#' Asymptotic Max-D test for testing equality means against tree-ordered alternatives in one-way ANOVA
#' @export
...
#' @details Testing of H_0:mu_0 = mu_1 = ... = mu_k vs H_1:mu_0 <= mu_i (at least one strict inequality), for all i >= 1, where mu_i represents the population means of the i-th treatment. The input consists of two variables: sample_data and significance_level. The output consists of the critical value, the AMax-D test statistic value, and the result, which indicates whether to reject or not reject the null hypothesis.
...
TreeAMaxD <- function(sample_data, significance_level){
  ...
  D_star_amax_values <- numeric(num_samples)
  for (k in 1:num_samples) {
    bootstrap_s <- MASS::mvrnorm(n = 1, mu = rep(0, s), Sigma = D)
    D_star_amax_values[k] <- max(bootstrap_s)
  }
  quantile_value <- quantile(sort(D_star_amax_values), probs = 1-significance_level)
  D_Amax <- max(sapply(2:num_datasets, function(j) (mean(sample_data[[j]]) - mean(sample_data[[1]])) /
                         sqrt((var(sample_data[[j]]) / length(sample_data[[j]])) +
                                (var(sample_data[[1]]) / length(sample_data[[1]])))))
  if (D_Amax > quantile_value) {
